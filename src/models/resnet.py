...
from .fractional_bias import relu_constant_fraction
...
class ResidualUnit_constant_fraction(nn.Module):

    def __init__(self, in_channel, out_channel, stride=1, activate_before_residual=False):
        super(ResidualUnit_constant_fraction, self).__init__()
    ...
    def forward(self, x):
        ...
        if self.activate_before_residual:
            orig_x = relu_constant_fraction(self.bn1(x))
        else:
            orig_x = x
            x = relu_constant_fraction(self.bn1(x))

        x = relu_constant_fraction(self.bn2(self.conv1(x)))
        x = self.conv2(x)
...
class ResNet_constant_fraction(nn.Module):

    def __init__(self, num_outputs=10):
        super(ResNet_constant_fraction, self).__init__()
        ...
        self.block1 = self.build_block(
            ResidualUnit_constant_fraction, 5, filters[0], filters[1], strides[0], True)
        self.block2 = self.build_block(
            ResidualUnit_constant_fraction, 5, filters[1], filters[2], strides[1], False)
        self.block3 = self.build_block(
            ResidualUnit_constant_fraction, 5, filters[2], filters[3], strides[2], False)
    ...
    def forward(self, x):
        ...
        out = self.block2(out)
        out = self.block3(out)
        out = relu_constant_fraction(self.bn1(out))
        out = F.avg_pool2d(out, out.size(-1))
        out = out.view(out.size(0), -1)
        out = self.linear(out)
...
class ResNetWide(nn.Module):
    ...
    def __init__(self, num_outputs=10):
        ...
        self.conv1 = nn.Conv2d(3, 160, kernel_size=3,
                               stride=1, padding=1, bias=False)
...
class ResNetWide_constant_fraction(nn.Module):

    def __init__(self, num_outputs=10):
        super(ResNetWide_constant_fraction, self).__init__()
    ...
    def forward(self, x):
        ...
        out = self.block1(out)
        out = self.block2(out)
        out = relu_constant_fraction(self.block3(out))
        out = F.avg_pool2d(out, 8)
        out = out.view(out.size(0), -1)
        out = self.linear(out)
